...
render after some defined timeout, while `pumpAndTrySettle()` does not. That's
why it has "`try`" in its name.
...
for some things to happen, but by using `pumpAndSettle`, you'll keep getting an
...
then a single `Scrollable` widget is visible at the same time. In such cases we
